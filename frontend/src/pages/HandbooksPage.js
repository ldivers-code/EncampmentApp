...
import {
  Plus, Edit2, Trash2, BookOpen, Download, FileText, File,
  Upload, Search, X, Loader2, FileSpreadsheet, Image, FileArchive,
  Presentation, Eye
} from 'lucide-react';
...
const CATEGORIES = [
  ...
  { value: 'training_guide', label: 'Training Guides' },
  { value: 'cadet_handbook', label: 'Cadet Handbook' },
...
const HandbooksPage = () => {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Delete this handbook?')) return;
    try {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            {filteredDocs.length} of {documents.length} handbooks
          </p>
        </div>
        {canUploadDocuments() && (
          <Button
            ...
            data-testid="upload-handbook-btn"
          >
      ...
      <div className="bg-white border border-slate-200 rounded-sm p-4 mb-6">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Select value={categoryFilter} onValueChange={setCategoryFilter}>
            <SelectTrigger className="w-full sm:w-48 rounded-sm" data-testid="handbook-category-filter">
      ...
      {filteredDocs.length === 0 ? (
        <div className="bg-white border border-slate-200 rounded-sm p-12 text-center">
          ...
          <p className="text-lg text-slate-500 font-medium">
            {documents.length === 0 ? 'No handbooks yet' : 'No matching handbooks'}
          </p>
          <p className="text-sm text-slate-400 mt-2">
            {documents.length === 0 ? 'Upload your first handbook to get started' : 'Try adjusting your search or filters'}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filteredDocs.map(doc => {
            const FileIcon = doc.storage_path ? getFileIcon(doc.file_type) : BookOpen;
            return (
              <div
                key={doc.id}
                className="bg-white border border-slate-200 rounded-sm hover:border-[#00205B]/30 hover:shadow-sm transition-all group"
                data-testid={`handbook-card-${doc.id}`}
              >
                <div className="p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        {doc.category && (
                          <span className="px-2 py-0.5 text-[10px] uppercase tracking-wider font-bold rounded-sm bg-[#00205B]/10 text-[#00205B] border border-[#00205B]/20">
                            {CATEGORIES.find(c => c.value === doc.category)?.label || doc.category}
                          </span>
                        )}
                        {doc.file_name && (
                          <span className="text-[10px] text-slate-400 truncate max-w-[150px]" title={doc.file_name}>
                            {doc.file_name}
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      {(doc.storage_path || doc.content) && canPreview(doc.file_type) && (
                        <Button
                          ...
                          data-testid={`preview-handbook-${doc.id}`}
                        >
                      ...
                      {(doc.storage_path || doc.file_url) && (
                        <Button
                          ...
                          className="h-8 text-xs rounded-sm"
                          data-testid={`download-handbook-${doc.id}`}
                        >
                      ...
                      {!doc.storage_path && !doc.file_url && doc.content && (
                        <Button
                          ...
                          onClick={() => setPreviewDoc(doc)}
                          className="h-8 text-xs rounded-sm"
                          data-testid={`view-handbook-${doc.id}`}
                        >
                          <Eye className="w-3 h-3 mr-1" />
                          View
                        </Button>
          ...
          <form onSubmit={handleUpload} className="space-y-4 mt-2">
            ...
            >
              <input
                ...
                data-testid="handbook-file-input"
              />
              {selectedFile ? (
            ...
            <div>
              ...
              <Input
                ...
                placeholder="e.g. Cadet Encampment Handbook"
            ...
            <div>
              ...
              <Select value={formData.category} onValueChange={(v) => setFormData(p => ({ ...p, category: v }))}>
                <SelectTrigger className="mt-1 rounded-sm" data-testid="handbook-category-select">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button type="submit" disabled={uploading} className="bg-[#00205B] hover:bg-[#001540] rounded-sm" data-testid="submit-handbook-btn">
          ...
          <form onSubmit={handleEditSubmit} className="space-y-4 mt-2">
            <div>
              ...
              <Input
                ...
                required
                className="mt-1 rounded-sm"
                data-testid="edit-handbook-title"
              />
            </div>
            <div>
              <Label className="text-xs uppercase tracking-wide text-slate-600">Category</Label>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button type="submit" className="bg-[#00205B] hover:bg-[#001540] rounded-sm" data-testid="save-edit-handbook-btn">Save Changes</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
