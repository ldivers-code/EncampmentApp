...
from permissions import (
    ...
    get_default_permissions, get_current_user, get_current_user_pending_ok, require_role
)
...
async def get_me(user: dict = Depends(get_current_user_pending_ok)):
    """Return the current user. Accessible to pending/unapproved users so the
    client can decide whether to show the app or the pending-approval screen."""
    user_data = {k: v for k, v in user.items() if k not in ("_id", "password_hash")}
    return UserResponse(**user_data)


@api_router.get("/auth/status")
async def get_account_status(user: dict = Depends(get_current_user_pending_ok)):
    """Structured account / participant linkage status — used by the client
    ...
    Fields:
      ...
      - encampment_participant_type: the participant_type from the linked
        roster record, or None when not linked
      - permission_role: the user's UserRole string
